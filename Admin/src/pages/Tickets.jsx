...
export default function Tickets() {
 ...
 <div>
 <h1 className="text-2xl font-bold text-blue-600 mb-1">Ticket Management</h1>
  </div>
 ...
 <div className="bg-white rounded-xl p-6 border border-blue-300 overflow-hidden">
 <p className="text-slate-500 text-sm font-medium mb-1">Tickets Today</p>
 <div className="flex items-end space-x-2">
 <h3 className="text-3xl font-bold text-slate-900">{stats.todayCount}</h3>
 <span className="text-sm font-medium text-blue-600 mb-1">Active</span>
 </div>
 </div>
 <div className="bg-white rounded-xl p-6 border border-blue-300 overflow-hidden">
 <p className="text-slate-500 text-sm font-medium mb-1">Pending Issuance</p>
 <div className="flex items-end space-x-2">
 <h3 className="text-3xl font-bold text-slate-90">{stats.unpaidCount}</h3>
 <span className="text-sm font-medium text-blue-600 mb-1">Urgent</span>
 </div>
 </div>
 <div className="bg-white rounded-xl p-6 border border-blue-300 overflow-hidden">
 ...
 <span className="text-sm font-medium text-blue-600 mb-1">Live</span>
 </div>
 </div>
 <div className="bg-white rounded-xl p-6 border border-blue-300 overflow-hidden">
 <p className="text-slate-500 text-sm font-medium mb-1">Refund Requests</p>
 <div className="flex items-end space-x-2">
 <h3 className="text-3xl font-bold text-slate-900">{stats.refundCount || 0}</h3>
 <span className="text-sm font-medium text-blue-600 mb-1">Last 24h</span>
 </div>
 </div>
 </div>

 {/* Issuance Trends moved below table */}

 <div className="bg-white rounded-xl p-4  overflow-hidden flex flex-wrap gap-4 items-center">
 <div className="flex-1 min-w-[200px] ">
 <label className="block text-xl font-semibold text-Black mb-1">Search </label>
 ...
 className="w-[400px] pl-9 pr-3 py-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 border border-slate-300 transition-shadow" 
 />
 </div>
 </div>
 <div className="w-48">
 <label className="block text-xs font-semibold text-slate-700 mb-1">Payment Status</label>
 <select value={filterPayment} onChange={(e) => setFilterPayment(e.target.value)} className="w-full rounded-lg px-3 py-2 text-sm text-slate-600 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/50">
 ...
 return (
 <tr key={t._id || t.id} className="hover:bg-blue-50/50 transition-colors border-b-0 last:border-0">
 <td className="px-6 py-4">
 <img src={t.photo || t.img || getAvatar(name, gender)} alt={name} className="w-10 h-10 rounded-full object-cover bg-white" />
 </td>
 <td className="px-6 py-4">
 <p className="font-medium text-slate-900">{name}</p>
 <p className="text-xs text-slate-500">Passport: {passport}</p>
 </td>
 <td className="px-6 py-4">
 <div className="flex items-center mb-1">
 <Plane className="w-4 h-4 text-slate-400 mr-2" />
 <span className="font-medium text-slate-900">{flightAirline}</span>
 </div>
 <p className="text-xs text-slate-500 pl-6">Flight #{flightNumber}</p>
 </td>
 <td className="px-6 py-4">
 <p className="font-bold text-slate-900">{t.currency || t.curr} {amount.toFixed(2)}</p>
 </td>
 <td className="px-6 py-4">
 <span className={`px-2.5 py-1 text-[10px] font-bold uppercase rounded-full ${getPaymentColor(payment)}`}>
 {payment}
 </span>
 </td>
 <td className="px-6 py-4 text-right">
 <button onClick={() => handleOpenEdit(t)} className="p-2 text-slate-400 hover:text-blue-600 transition-colors">
 <Edit className="w-4 h-4" />
 </button>
 <button onClick={() => handleDelete(t._id || t.id)} className="p-2 text-slate-400 hover:text-slate-500 transition-colors ml-1">
 ...
 <div className="lg:col-span-2 bg-white rounded-xl p-6 border border-slate-200 overflow-hidden">
