...
import { usePermissions } from '../context/AuthContext';
 ...
 name: '', country: '', durationDays: 30, basePrice: 0, description: ''
 });
 ...
 const handleOpenAdd = () => {
 setEditingId(null);
 setFormData({ name: '', country: '', durationDays: 30, basePrice: 0, description: '' });
 setIsModalOpen(true);
 };
 ...
 alert("Failed to delete Visa Type");
 }
 };
 ...
 try {
 if (editingId) {
 await updateVisaType(editingId, formData);
 } else {
 ...
 const filteredData = visaTypes.filter(vt => 
 ...
 {filteredData.map((vt) => (
 <tr key={vt._id} className="hover:bg-white transition-colors">
 ...
 <div>
 <p className="font-medium text-slate-900">{vt.name}</p>
 {vt.description && <p className="text-xs text-slate-500 truncate max-w-[200px]">{vt.description}</p>}
 </div>
 </div>
 </td>
 ...
 <tr>
 <td colSpan="5" className="px-6 py-8 text-center text-slate-500">No visa types found.</td>
 </tr>
 )}
 </tbody>
 </table>
 ...
 <h2 className="text-2xl font-bold text-slate-900">{editingId ? 'Edit Visa Type' : 'Add Visa Type'}</h2>
 ...
 <div>
 <label className="block text-sm font-semibold text-slate-700 mb-2">Visa Name</label>
 <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} type="text" placeholder="e.g. Tourist Visa" className="w-full px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-shadow" />
 </div>
 <div>
 <label className="block text-sm font-semibold text-slate-700 mb-2">Country</label>
 <input required value={formData.country} onChange={e => setFormData({...formData, country: e.target.value})} type="text" placeholder="e.g. UAE" className="w-full px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-shadow" />
 </div>
 </div>
 ...
 <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Optional details..." rows="3" className="w-full px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-shadow"></textarea>
 </div>
 ...
 {editingId ? 'Update Visa Type' : 'Save Visa Type'}
 </button>
 </div>
 </form>
 </div>
